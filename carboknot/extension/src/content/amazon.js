// Carboknot — Amazon product-page adapter.
//
// Zero-trust invariants enforced here:
//   - No fetch, no XHR, no analytics. Everything below runs on-device.
//   - All state writes go through storage/db.js::logView.
//   - The trace object returned by computeCarbon is passed through verbatim;
...
// Regex patterns sufficient to classify the five demo products; anything
// that does not match falls through to the 'general' bucket, which the
// engine widens to ±40% by design.
// Patterns anchor on word-start only (no trailing \b) so that regular
...
// Amazon re-renders the price area on variant selection. Watch briefly.
